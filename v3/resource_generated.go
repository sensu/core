...
// SetMetadata sets the provided metadata on the type. If the type does not
// have any metadata, nothing will happen.
func (e *EntityKeepalive) SetMetadata(meta *corev2.ObjectMeta) {
	// The function has to use reflection, since not all of the generated types
	// will have metadata.
	value := reflect.Indirect(reflect.ValueOf(e))
	field := value.FieldByName("Metadata")
...
// StoreName returns the store name for EntityKeepalive. It will be
// overridden if there is a method for EntityKeepalive called "storeName".
func (e *EntityKeepalive) StoreName() string {
	var iface interface{} = e
	if prefixer, ok := iface.(storeNamer); ok {
		return prefixer.storeName()
	}
	return "entity_keepalives"
}

// RBACName returns the RBAC name for EntityKeepalive. It will be overridden if
// there is a method for EntityKeepalive called "rbacName".
func (e *EntityKeepalive) RBACName() string {
	var iface interface{} = e
	if namer, ok := iface.(rbacNamer); ok {
		return namer.rbacName()
	}
...
// URIPath returns the URI path for EntityKeepalive. It will be overridden if
// there is a method for EntityKeepalive called uriPath.
func (e *EntityKeepalive) URIPath() string {
	var iface interface{} = e
	if pather, ok := iface.(uriPather); ok {
		return pather.uriPath()
	}
	...
	meta := metaer.GetMetadata()
	if meta == nil {
		return uriPath("entity-keepalives", "", "")
	}
	return uriPath("entity-keepalives", meta.Namespace, meta.Name)
}

// Validate validates the EntityKeepalive. If the EntityKeepalive has metadata,
// it will be validated via ValidateMetadata. If there is a method for
// EntityKeepalive called validate, then it will be used to cooperatively
// validate the EntityKeepalive.
func (e *EntityKeepalive) Validate() error {
	if e == nil {
		return errors.New("nil EntityKeepalive")
	}
	var iface interface{} = e
	if resource, ok := iface.(Resource); ok {
		meta := resource.GetMetadata()
		if err := ValidateMetadata(meta); err != nil {
			return fmt.Errorf("invalid EntityKeepalive: %s", err)
		}
		if gr, ok := iface.(GlobalResource); ok && gr.IsGlobalResource() {
			if err := ValidateGlobalMetadata(meta); err != nil {
				return fmt.Errorf("invalid EntityKeepalive: %s", err)
...
// UnmarshalJSON is provided in order to ensure that metadata labels and
// annotations are never nil.
func (e *EntityKeepalive) UnmarshalJSON(msg []byte) error {
	type Clone EntityKeepalive
	var clone Clone
	if err := json.Unmarshal(msg, &clone); err != nil {
		return err
	}
	*e = *(*EntityKeepalive)(&clone)
	var iface interface{} = e
	var meta *corev2.ObjectMeta
	if metaer, ok := iface.(getMetadataer); ok {
		meta = metaer.GetMetadata()
...
// GetTypeMeta gets the type metadata for a EntityKeepalive.
func (e *EntityKeepalive) GetTypeMeta() corev2.TypeMeta {
	return corev2.TypeMeta{
		APIVersion: "core/v3",
		Type:       "EntityKeepalive",
	}
}
