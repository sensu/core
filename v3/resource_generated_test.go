...
func TestEntityKeepaliveSetMetadata(t *testing.T) {
	value := new(EntityKeepalive)
	var iface interface{} = value
	metaer, ok := iface.(getMetadataer)
	if !ok {
		return
...
func TestEntityKeepaliveStoreName(t *testing.T) {
	var value EntityKeepalive
	got := value.StoreName()
	if len(got) == 0 {
		t.Error("undefined store suffix")
	}
...
func TestEntityKeepaliveRBACName(t *testing.T) {
	var value EntityKeepalive
	got := value.RBACName()
	if len(got) == 0 {
		t.Error("undefined rbac name")
...
func TestEntityKeepaliveURIPath(t *testing.T) {
	var value EntityKeepalive
	value.Metadata = &corev2.ObjectMeta{
		Namespace: "default",
		Name:      "foo",
...
func TestEntityKeepaliveValidate(t *testing.T) {
	var value EntityKeepalive
	if err := value.Validate(); err == nil {
		t.Errorf("expected non-nil error for nil metadata")
	}
...
func TestEntityKeepaliveUnmarshalJSON(t *testing.T) {
	msg := []byte(`{"metadata": {"namespace": "default", "name": "foo"}}`)
	var value EntityKeepalive
	if err := json.Unmarshal(msg, &value); err != nil {
		t.Fatal(err)
...
func TestEntityKeepaliveGetTypeMeta(t *testing.T) {
	var value EntityKeepalive
	meta := value.GetTypeMeta()
	if got, want := meta.APIVersion, "core/v3"; got != want {
		t.Errorf("bad api version: got %s, want %s", got, want)
	}
	if got, want := meta.Type, "EntityKeepalive"; got != want {
		t.Errorf("bad type: got %s, want %s", got, want)
	}
}
